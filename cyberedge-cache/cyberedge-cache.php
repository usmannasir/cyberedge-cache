...
final class CyberEdge_Config {
    ...
    public static function load() {
        foreach ( array( 'CYBEREDGE_SITE_ID', 'CYBEREDGE_CONTROLLER_URL', 'CYBEREDGE_PURGE_SECRET' ) as $name ) {
            if ( ! defined( $name ) ) {
                throw new InvalidArgumentException( 'CyberEdge server configuration is missing.' );
            }
        }
...
final class CyberEdge_Cache {
    ...
    public function enqueue( $reason ) {
        try {
            ...
            return $this->outbox->enqueue( $this->configuration(), $reason, call_user_func( $this->clock ) );
    ...
    public function admin_notice() {
        ...
        if ( $bad ) {
            echo '<div class="notice notice-error"><p>CyberEdge purge delivery needs attention. Check the server configuration and run wp cyberedge status. Cached pages may remain stale until their TTL expires.</p></div>';
        }
    }
    ...
    public function admin_assets( $hook ) {
        ...
        wp_enqueue_style( 'cyberedge-cache-admin', plugins_url( 'assets/admin.css', __FILE__ ), array(), '0.3.0' );
        wp_enqueue_script( 'cyberedge-cache-admin', plugins_url( 'assets/admin.js', __FILE__ ), array(), '0.3.0', true );
    ...
    public function status_page() {
        ...
        echo '</form><p class="description">No purge secret or controller credential is shown on this page.</p></section></div>';
    }
...
function cyberedge_activate( $network_wide = false ) {
    ...
    try { CyberEdge_Config::load(); } catch ( Throwable $error ) { wp_die( esc_html( $error->getMessage() ) ); }
    global $wpdb;
    ...
    if ( ! $cache->enqueue( 'activation' ) ) { wp_die( 'CyberEdge could not create its durable purge outbox.' ); }
}

register_activation_hook( __FILE__, 'cyberedge_activate' );
register_deactivation_hook( __FILE__, function () { wp_clear_scheduled_hook( CyberEdge_Cache::CRON ); } );
global $wpdb;
