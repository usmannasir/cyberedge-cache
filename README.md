...
When LSCWP is loaded, server-supported, enabled, and not globally disabled, CyberEdge leaves public cacheability to LSCWP. Merely installing LSCWP for optimization does not disable the CyberEdge fallback. CyberEdge adds a no-cache/no-store veto for authenticated or cookie-bearing requests, any query string, non-GET/HEAD methods, previews, search, errors, feeds, AJAX/REST, password-protected content, WooCommerce cart/checkout/account pages, and `DONOTCACHEPAGE`. Existing private/no-store/ESI directives, Set-Cookie, and non-HTML responses veto the fallback. Without active LSCWP page caching, remaining anonymous HTML receives `X-LiteSpeed-Cache-Control: public,max-age=300`. Custom personalization that does not use cookies/auth or cache-control signals still requires application integration. Public fallback can be disabled with TTL zero.
